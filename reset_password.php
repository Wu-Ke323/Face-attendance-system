...
$sql2= "UPDATE teacher SET te_password='$password',te_email='$e'  WHERE te_id='$username'";
 //插入数据库
 if(!(mysqli_query($link,$sql2))){
 echo "<script>alert('完善信息失败');window.location.href='reset_password.html'</script>";
 }
 else{
 echo  "<script>alert('完善信息成功！欢迎进入考勤系统!');window.location.href='teacher1.html'</script>";
 }
}
//student
else{
...
$sql2= "UPDATE student SET st_password='$password',st_email='$e'  WHERE st_id='$username'";
 //更新数据库
if(!(mysqli_query($link,$sql2)))  {
	echo "<script>alert('完善信息失败');window.location.href='reset_password.html'</script>";//.mysqli_error($link);
}
else{
 echo  "<script>alert('完善信息成功，欢迎进入考勤系统!');window.location.href='student1.html'</script>";
 }
mysqli_close($link);
}
?>
