...
session_start();
$id=$_SESSION['id'];
//获取teacher1传来的course_name并找到对应的course_id
$course_id=$_GET['course_id'];
$_SESSION['course_id']=$course_id;

//根据course_id从数据库获取course_name
$link = mysqli_connect("localhost", "root",'jzyssbxf5255','face attendance');
...
//显示该课程总人数
    $link_1 = mysqli_connect("localhost", "root",'jzyssbxf5255','face attendance');
...
function news($pageNum = 1, $pageSize = 3)
{
	...
	//$course_id=201801;
    $array = array();
    $coon = mysqli_connect("localhost", "root",'jzyssbxf5255','face attendance');
    mysqli_select_db($coon, "face attendance");
    mysqli_set_charset($coon, "utf8");
	$nowdate=date("Y-m-d");
    // limit为约束显示多少条信息，后面有两个参数，第一个为从第几个开始，第二个为长度
    $rs = "select * from teacher_course_attendance where date<='$nowdate' and course_id='$course_id' order by date DESC limit " . (($pageNum - 1) * $pageSize) . "," . $pageSize;
...
function allNews()
{
    ...
    mysqli_set_charset($coon, "utf8");
	$nowdate=date("Y-m-d");
    $rs = "select count(*) num from teacher_course_attendance where date<='$nowdate' and course_id='$course_id' " ; //可以显示出总页数
    $r = mysqli_query($coon, $rs);
