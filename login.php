...
$dbpass = 'jzyssbxf5255';          // mysql用户名密码
...
while($row = mysqli_fetch_array($retval, MYSQLI_ASSOC))
{
   if ($username == $row["te_id"] && $password == $row["te_password"]&&$username!=$password) {
	   ...
	   $_SESSION['id']=$username;
		 echo "<script type='text/javascript'>alert('登陆成功');location='teacher1.html';</script>";exit(0);
   }
...
while($row = mysqli_fetch_array($retval, MYSQLI_ASSOC))
{
   if ($username == $row["st_id"] && $password == $row["st_password"]&&$username!=$password) {
		 ...
		 //echo "验证成功！<br>";
	   	   $_SESSION['id']=$username;
		 echo "<script type='text/javascript'>alert('登陆成功');location='student1.html';</script>";exit(0);
		} 
